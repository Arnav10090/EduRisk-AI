...
class PredictionService:
    ...
    async def _create_prediction_record(
        ...
        prediction = Prediction(
            ...
            risk_score=risk_score,
            risk_level=risk_level,
            salary_min=Decimal(str(salary_pred.salary_min)) if salary_pred.salary_min is not None else None,
            salary_max=Decimal(str(salary_pred.salary_max)) if salary_pred.salary_max is not None else None,
            salary_confidence=Decimal(str(salary_pred.confidence)) if salary_pred.confidence is not None else None,
            emi_affordability=Decimal(str(emi_affordability)) if emi_affordability is not None else None,
            shap_values=shap_values,
            top_risk_drivers=top_risk_drivers,
            ai_summary=ai_summary,
